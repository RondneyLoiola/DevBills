import { AlertCircle, ArrowDown, ArrowUp, PlusIcon, SearchIcon, Trash2 } from "lucide-react"
...
import { formartCurrency, formatDate } from "../utils/formatters"
...
const Transactions = () => {
    ...
    const fecthTransactions = async () => {
        try {
            setLoading(true)// liga o loading quando faz a requisição
    ...
    return (
        <div className="container-app py-6">
            <div className="flex flex-col md:flex-row items-start justify-between md:items-center mb-6">
                <h1 className="text-2xl font-bold mb-4 md:mb-0">Transações</h1>
                <Link to='/transacoes/nova' className="bg-primary-500 text-[#051626] font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center hover:bg-primary-600 transition-colors">
            ...
            <Card className="mb-6">
                ...
                fullWidth
                onChange={handleSearchChange} //chama toda vez que digita
                value={searchText}
                />
            </Card>
            ...
            <Card className="overflow-hidden">
                {loading ? (
                    <div className="flex items-center justify-center">
                        <div className="w-10 h-10 m-2 border-4 border-primary-500 border-t-transparent rounded-full animate-spin"/>
                    </div>
                ): error ? (
                    <div className="p-8 text-center">
                        <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4"/>
                        <p className="text-red-500">{error}</p>
                        <Button onClick={fecthTransactions} className="mx-auto mt-6">Tentar Novamente</Button>
                    </div>
                ): transactions?.length === 0 ? (
                    <div className="text-center py-12">
                        <p className="text-gray-500 mb-4">Nenhuma Transação encontrada.</p>

                        <Link to='/transacoes/nova' className="w-fit mt-6 mx-auto bg-primary-500 text-[#051626] font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center hover:bg-primary-600 transition-colors">
                            <PlusIcon className="w-4 h-4 mr-2" />
                            Nova Transação
                        </Link>
                    </div>
                                    ...
                                    <tr key={transaction.id} className="hover:bg-gray-800">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`${transaction.type === TransactionType.INCOME ? 'text-primary-500' : 'text-red-500'}`}>
                                                {formartCurrency(transaction.amount)}
                                            </span>
                                        </td>
