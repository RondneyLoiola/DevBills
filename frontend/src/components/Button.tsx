...
const Button = ({
    ...
    return (
        <button type="button" {...rest} className={`cursor-pointer px-5 py-2.5 rounded-xl font-medium transition-all flex items-center justify-center
            ${variantClasses[variant]}
            ${className}
            ${fullWidth ? 'w-full' : ''} 
            ${isLoading || disabled ? 'opacity-70 cursor-not-allowed' : ''}
            `} disabled={isLoading || disabled}>{isLoading ? renderLoading() : children}</button>
    )
}
