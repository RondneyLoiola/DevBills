...
export const getTransactionsSummary = async (
    ...
    //como é um resumo de transações, precisa de mês e ano
    const {month, year} = request.query 

    if(!month || !year) { //se não tiver mês ou ano
    ...
    try {
        const transactions = await prisma.transaction.findMany({
            ...
            include: {
                category: true // trás tudo da categoria
            }
        })
        ...
        const summary: TransactionSummary = {
            ...
            })).sort((a, b) => b.amount - a.amount) // sort -> organizar o array
        }
